...
int wm_parse_attr_block(const char *s, int len, int *pos, wm_parsed_attrs *out) {
    ...
    int i = wm_scan_to_delim(s, len, start, '}');
    if (i < 0) return 0;
    ...
    *pos = i + 1;
    ...
    while (p < end) {
        ...
        if (s[p] == '.') {
            ...
            if (out->class_count < WM_MAX_ATTRS) {
...
void wm_attach_attributes(cmark_node *root, cmark_mem *mem) {
    ...
    while ((ev_type = cmark_iter_next(iter)) != CMARK_EVENT_DONE) {
        ...
        if (ev_type == CMARK_EVENT_ENTER && node->type == CMARK_NODE_TEXT && prev_elem) {
            ...
            if (lit && (lit[0] == '{' || lit[0] == '|')) {
                if (count >= cap) {
                    cap = cap ? cap * 2 : 16;
                    void *tmp = realloc(pairs, cap * sizeof(pair));
                    if (!tmp) break;
                    pairs = tmp;
                }
    ...
    for (int i = 0; i < count; i++) {
        ...
        while (pos < len && lit[pos] == '{') {
            wm_parsed_attrs more;
            memset(&more, 0, sizeof(more));
            if (!wm_parse_attr_block(lit, len, &pos, &more)) break;
            /* Merge into attrs */
            for (int j = 0; j < more.class_count && attrs.class_count < WM_MAX_ATTRS; j++)
                attrs.classes[attrs.class_count++] = more.classes[j];
            if (more.id) { free(attrs.id); attrs.id = more.id; }
            for (int j = 0; j < more.kv_count; j++) {
                /* Later values override earlier ones for the same key */
                int replaced = 0;
                for (int k = 0; k < attrs.kv_count; k++) {
                    if (strcmp(attrs.keys[k], more.keys[j]) == 0) {
                        free(attrs.values[k]);
                        attrs.values[k] = more.values[j];
                        free(more.keys[j]);
                        replaced = 1;
                        break;
                    }
                }
                if (!replaced && attrs.kv_count < WM_MAX_ATTRS) {
        ...
        /* Parse |..| block */
        if (pos < len && lit[pos] == '|') {
            wm_parse_annotation_block(lit, len, &pos, &annot);
        }
        ...
        if (!html) { free(link_text); wm_free_attrs(&attrs); wm_free_annotation(&annot); continue; }
        size_t hlen = 0;

        #define APPEND(s) do { \
            size_t _l = strlen(s); \
            while (hlen + _l >= html_cap) { \
                html_cap *= 2; \
                void *_t = realloc(html, html_cap); \
                if (!_t) break; \
                html = (char *)_t; \
            } \
        ...
        if (is_image) {
            ...
            if (parent && parent->type == CMARK_NODE_PARAGRAPH) {
                 ...
                 * AND the modifier text has no remaining content after modifiers */
                if (first == elem && (!after_elem || after_elem == text)) {
                    if (!after_elem) {
                        /* No text node follows — standalone */
                        is_figure = 1;
                    } else if (pos >= len && !cmark_node_next(text)) {
                        /* Text node fully consumed by modifiers, nothing after — standalone */
                        is_figure = 1;
                    }
                    /* Otherwise, content remains after modifiers — not standalone */
                }
            }
        }
        ...
        if (is_figure) {
            ...
            if (only_child) {
                ...
                if (hlen + 2 >= html_cap) { html_cap *= 2; void *_t = realloc(html, html_cap); if (_t) html = (char *)_t; }
                html[hlen++] = '\n';
                html[hlen] = '\0';
