...
/**
 * Replace U+E000 PUA sentinels back to '[' in a string.
 * Only handles U+E000 (brackets) — U+E001 (braces) is restored later
 * by wm_restore_escaped_braces() after template processing.
...
static cmark_node *postprocess(cmark_syntax_extension *ext,
    ...
    wm_parse_state *state = wm_current_parse_state;
        ...
        if (ev_type == CMARK_EVENT_ENTER && node->type == CMARK_NODE_TEXT
            ...
            if (lit && strstr(lit, "[[")) {
                if (node_count >= node_cap) {
                    ...
                    void *tmp = realloc(nodes, node_cap * sizeof(cmark_node *));
                    if (!tmp) break;
                    nodes = tmp;
                }
    ...
    /* --- Pass 2: Rewrite wiki-style links AND restore escaped brackets ---
     * Combined into a single AST walk. Wiki-style links (scheme-less) get
     * their URLs normalized. U+E000 sentinels get restored to '['. */
    {
        cmark_iter *iter2 = cmark_iter_new(root);
        cmark_event_type ev2;

        while ((ev2 = cmark_iter_next(iter2)) != CMARK_EVENT_DONE) {
            cmark_node *node = cmark_iter_get_node(iter2);
            if (ev2 != CMARK_EVENT_ENTER) continue;
            ...
            if (node->type == CMARK_NODE_LINK) {
                ...
                if (is_wiki_url(url)) {
                    ...
                    if (normalized) {
                        cmark_node_set_url(node, normalized);
                        mem->free(normalized);
                    }
                }
            } else if (node->type == CMARK_NODE_TEXT) {
                const char *lit = cmark_node_get_literal(node);
                if (lit) {
